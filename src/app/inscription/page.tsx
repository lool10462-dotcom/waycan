...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("Compte créé avec succès ! Si vous avez activé la confirmation d'email sur Supabase, veuillez vérifier votre boîte de réception.")
      router.push('/')
    } catch (err: any) {
  ...
  const getPasswordStrength = () => {
    ...
    if (password.length < 6) return { level: 1, text: 'Faible', color: 'bg-alert' }
    if (password.length < 10 || !/[A-Z]/.test(password)) return { level: 2, text: 'Moyen', color: 'bg-premium-gold' }
    return { level: 3, text: 'Fort', color: 'bg-success' }
  }
      ...
      <div className="max-w-md mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-surface rounded-2xl p-8">
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold text-text-main">Créer un compte</h1>
            <p className="text-text-muted mt-2">Rejoignez WAYCAN et commencez à vendre</p>
          </div>
